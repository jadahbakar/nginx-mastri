...
use App\Events\Konsolidasi\NotifikasiLaporanKonsolidasiPrognosisSapEvent;
use App\Models\Pengaturan\TandaTangan;

class LaporanKonsolidasiPrognosisSapJob implements ShouldQueue
{
    ...
    {
        Log::info('Mulai proses laporan konsolidasi prognosis SAP skpd '.$dataQueue['kodeOrganisasi']);
    ...
    public function handle()
    {
        ...
        try {
            ...
            $pdf = PDF::loadView('konsolidasi.prognosis.lra-sap.lra-sap-print', compact('var', 'helper', 'tandaTangan'));
            $namaFile = date("DdMY").'_'.time().'_Laporan Konsolidasi Prognosis SAP_'.$kodeOrganisasi.'.pdf';
            if(env('APP_ENV') == 'production'){
                $pdf->save('/usr/share/nginx/html/ereport-2019/public/laporan/konsolidasi/prognosis/laporan-sap/'.$namaFile);
            }else{
                $pdf->save('public/laporan/konsolidasi/prognosis/laporan-sap/'.$namaFile);
            }
            $direktoriFile =  '/laporan/konsolidasi/prognosis/laporan-sap/'.$namaFile;
            $urlFile =  url('/').'/laporan/konsolidasi/prognosis/laporan-sap/'.$namaFile;
            ...
            Event::fire(new NotifikasiLaporanKonsolidasiPrognosisSapEvent($this->dataEvent));
            Log::info('Laporan Konsolidasi Prognosis SAP sukses');
        } catch(\Exception $e) {
    ...
    {
        Event::fire(new NotifikasiLaporanKonsolidasiPrognosisSapEvent($this->dataEvent));
        Log::error('Laporan Konsolidasi Prognosis SAP gagal: '.$e->getMessage());
    }
}
