...
@section('content')
    <div class="content-header row">
        <div class="content-header-left col-md-6 col-12 mb-2">
          	<h3 class="content-header-title">LRA Penjabaran Prognosis</h3>
          	<div class="row breadcrumbs-top">
            	<div class="breadcrumb-wrapper col-12">
      				<ol class="breadcrumb">
        				<li class="breadcrumb-item"><a href="#">Beranda</a></li>
        				<li class="breadcrumb-item"><a href="#">Prognosis</a></li>
        				<li class="breadcrumb-item active"><a href="#">LRA Penjabaran Prognosis</a></li>
      				</ol>
            	</div>
          	</div>
        </div>
                		...
                		<div class="card-content">
                  			<div class="card-body">
                    			<div class="tab-content px-1 pt-1">
                      				<br />
                                    {!! Form::open(['id'=>'form-cetak', 'method'=>'POST', 'url'=>'prognosis/laporan/lra-penjabaran-prognosis', 'class'=>'form form-horizontal']) !!}
                                        @if(Auth::user()->view == 1)
...
@section('javascript')
	<script type="text/javascript">
        @if(Auth::user()->view == 3)
            socket.on("channel-name-{!! Auth::user()->kode_pengguna !!}:App\\Events\\Prognosis\\NotifikasiLaporanPenjabaranPrognosisEvent", function(message){
        ...
        @elseif(Auth::user()->view == 2)
            socket.on("channel-name-{!! Auth::user()->organisasi !!}:App\\Events\\Prognosis\\NotifikasiLaporanPenjabaranPrognosisEvent", function(message){
                $("#status-"+message.idProsesLaporan).html(message.labelHtml);
                if(message.status == 'Sukses'){
                    var urlFile = '{!! url('/') !!}'+message.direktoriFile;
                    $("#buttonDownload-"+message.idProsesLaporan).removeClass('disabled').attr("href", urlFile);
        ...
        @elseif(Auth::user()->view == 1)
            socket.on("channel-name-main-admin:App\\Events\\Prognosis\\NotifikasiLaporanPenjabaranPrognosisEvent", function(message){
                $("#status-"+message.idProsesLaporan).html(message.labelHtml);
                if(message.status == 'Sukses'){
                    var urlFile = '{!! url('/') !!}'+message.direktoriFile;
                    $("#buttonDownload-"+message.idProsesLaporan).removeClass('disabled').attr("href", urlFile);
